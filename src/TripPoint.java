...
public class TripPoint {
	...
	/**
	 * Detects stops in a trip log by comparing the distance between consecutive points.
	 * 
	 * @throws IOException if an I/O error occurs while reading the trip log file.
	 * @throws FileNotFoundException if the trip log file cannot be found.
	 * @return the number of stops detected in the trip.
	 */
	public static int h1StopDetection() throws IOException, FileNotFoundException {
		if (trip == null) {
		...
		for (int i = indexArray.size()-1; i >= 0; i--) {
			movingTrip.remove(indexArray.get(i).intValue());
		}
	...
	/**
	 * Returns a copy of the moving trip log.
	 * 
	 * @return an ArrayList of TripPoint objects representing the moving trip log.
	 */
